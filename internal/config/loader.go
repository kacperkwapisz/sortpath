...
func Set(key, value string) error {
	...
	case "last-update-check":
		...
		if err == nil {
			...
			return errors.New("invalid time format, expected RFC3339")
		}
	default:
...
func Get(key string) (string, error) {
	...
	case "last-update-check":
		if !c.LastUpdateCheck.IsZero() {
			return c.LastUpdateCheck.Format(time.RFC3339), nil
		}
		return "", nil
	default:
		return "", errors.New("unknown config key")
...
func (c *Config) ToMap() map[string]string {
	return map[string]string{
		...
		"last-update-check": func() string {
			if !c.LastUpdateCheck.IsZero() {
				return c.LastUpdateCheck.Format(time.RFC3339)
			}
			return ""
		}(),
	}
